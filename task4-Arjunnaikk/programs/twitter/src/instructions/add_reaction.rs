...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    match reaction {
        ReactionType::Like => match tweet.likes.checked_add(1) {
            Some(value) => {
                tweet.likes = value;
                Ok(())
            }
            None => err!(TwitterError::MaxLikesReached),
        },
        ReactionType::Dislike => match tweet.dislikes.checked_add(1) {
            Some(value) => {
                tweet.dislikes = value;
                Ok(())
            }
            None => err!(TwitterError::MaxDislikesReached),
        },
    }
}
